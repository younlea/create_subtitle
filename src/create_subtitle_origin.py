...
import os
import whisper
# from googletrans import Translator # googletrans 라이브러리 주석 처리
...
def translate_text(text, source_lang='ja', target_lang='ko'):
    ...
    print("번역 기능이 일시적으로 비활성화되었습니다. 원본 텍스트를 반환합니다.")
    return text # 번역 대신 원본 텍스트 반환
...
def create_srt_file(transcription_result, output_srt_path):
    with open(output_srt_path, 'w', encoding='utf-8') as f:
        for i, segment in enumerate(transcription_result['segments']):
            ...
            translated_text = translate_text(original_text) # 번역 함수 호출은 유지하되, 위에서 원본 텍스트 반환
